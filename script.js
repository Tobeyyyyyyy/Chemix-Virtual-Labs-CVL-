...
// Drawer drag state: { item, startX, startY, dragging, ghostEl }
let cardDrag = null;
...
function drawPlacedItems() {
  ...
  placedItems.forEach((item) => {
    ...
    ctx.font = 'bold 24px ui-monospace, "SF Mono", Menlo, monospace';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.shadowColor = accent;
    ctx.shadowBlur = 12;
    ctx.fillText(item.symbol, item.worldX, item.worldY - 14);
    ctx.restore();
    ...
    const lineH = 13;
    const startY = item.worldY + 20 - ((Math.min(lines.length, 2) - 1) * lineH) / 2;
...
window.addEventListener('mousemove', (e) => {
  // 1. Drawer-card drag: threshold check
  if (cardDrag && !cardDrag.dragging) {
    ...
    if (Math.hypot(dx, dy) > 6) {
      ...
      ghost.innerHTML = `<span class="ghost-symbol">${cardDrag.item.symbol}</span><span class="ghost-name">${cardDrag.item.name}</span>`;
  ...
  if (cardDrag) {
    if (cardDrag.dragging) {
      ...
      if (onCanvas) {
        const w = screenToWorld(e.clientX, e.clientY);
        const newItem = {
          symbol: cardDrag.item.symbol,
          name: cardDrag.item.name,
          worldX: w.x,
          worldY: w.y,
          type: activeCategory,
          id: 'chem-' + Date.now() + '-' + Math.random().toString(36).slice(2, 7),
        };
      ...
      // Was a click → open size picker or log
...
function handleCardClick(item) {
  if (item.sizes && item.sizes.length > 0) {
    ...
    console.log('Selected:', item.symbol || item.name);
  }
}
...
function renderDrawer() {
  ...
  drawerBody.querySelectorAll('.tool-card').forEach((card, idx) => {
    ...
    if (item.symbol) {
      // Chemical card → mousedown begins press, mouseup decides click vs drop
      ...
      // Non-chemical card → plain click opens size picker
...
function openSizePicker(item) {
  ...
  sizeListEl.querySelectorAll('.size-chip').forEach((chip) => {
    chip.addEventListener('click', () => {
      console.log('Selected:', item.name, '—', chip.dataset.size);
      closeSizePicker();
    });
  });
