...
(function initIntroScreen() {
  ...
  try {
    ...
    if (saved && contBtn) {
      ...
      if (data && (data.placedItems?.length || data.view)) contBtn.hidden = false;
    }
  } catch (e) {}
...
function hitTestPlaced(wx, wy) {
  for (let i = placedItems.length - 1; i >= 0; i--) {
    ...
    if (wx >= it.worldX - ITEM_W / 2 && wx <= it.worldX + ITEM_W / 2 &&
        wy >= it.worldY - ITEM_H / 2 && wy <= it.worldY + ITEM_H / 2) return it;
  }
  return null;
}
...
window.addEventListener('mousemove', (e) => {
  if (cardDrag && !cardDrag.dragging) {
    ...
    if (Math.hypot(dx, dy) > 6) startCardDrag(cardDrag, e.clientX, e.clientY);
  }
  ...
  if (e.clientX >= rect.left && e.clientX <= rect.right &&
    ...
    if (viewLocked) canvas.style.cursor = 'not-allowed';
